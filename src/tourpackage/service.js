...
const getSingleTourPackages = async (PKID) => {
  ...
  `;
    ...
    const tourPackageData = {
      ...
      infantinclusion: tourPackageResults[0].infantinclusion,
      infantexclusion: tourPackageResults[0].infantexclusion,
  ...
  try {
    const tourPackageQuery = `
      SELECT
        ...
        tourpackage.infantinclusion,
        tourpackage.infantexclusion,
...
const deleteBOOKINGSLOT = async (req, res) => {
  ...
  await pool.query(deletequery, [id])
  const deleteinstallmentQuery = `DELETE FROM installment WHERE bookingslotid= ?`
    ...
    for (const slotData of bookingSlotData) {
      ...
      if (bookingslotid) {
        ...
        await connection.query(
          ...
          [StartDate, EndDate, cancellationDate, available_seat, soldOut,bookingslotid, tourPackageId]
        );
      } else {
